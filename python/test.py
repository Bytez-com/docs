...
class TestBytez(unittest.TestCase):
  ...
  def test_starts(self):
    ...
    if response['status'] != 'error':
        ...
        alreadyStarted = "already loaded" in response['error'] or "operation already in progress: load" in response['error']
        
        self.assertEqual(alreadyStarted, True, "model already started")

    status = model.status()
    
    self.assertIn(status['status'], ["DEPLOYING", "RUNNING"], "returns status deploying or running")
  ...
  def test_runs_model(self):
    ...
    print(response)
    self.assertIsInstance(response['output'][0]['generated_text'], str, "returns output")
  
  def test_runs_model_with_params(self):
    """Test streaming output."""
    input = "Jack and Jill "
    response = model.run(input, model_params={"max_new_tokens":1,"min_new_tokens":1})
    print(response)
    output = response['output'][0]['generated_text']

    self.assertEqual(len(input.strip().split(' ')) + 1, len(output.split(' ')), "streams output")
